...
const KEY_STRATEGY_OPTIONS: { value: KeyStrategy; label: () => string; hint: () => string }[] = [
  { value: 'round_robin', label: m.key_pool_strategy_rr, hint: m.key_pool_strategy_rr_hint },
  { value: 'sticky', label: m.key_pool_strategy_sticky, hint: m.key_pool_strategy_sticky_hint },
  { value: 'random', label: m.key_pool_strategy_random, hint: m.key_pool_strategy_random_hint },
]
...
<template>
  <section class="glass glass-specular p-5">
    <div class="mb-1 flex items-center justify-between">
      <h2 class="text-sm font-semibold text-ink-soft uppercase">{{ m.key_pool_title() }}</h2>
      <button
        type="button"
        class="rounded px-2 py-1 text-xs text-ink-faint hover:text-ink cursor-pointer"
        :aria-pressed="showCredential"
        @click="showCredential = !showCredential"
      >
        {{ showCredential ? m.common_hide() : m.common_show() }}
      </button>
    </div>
    ...
    <div class="mt-4">
      <textarea
        ...
        placeholder="sk-...&#10;sk-...&#10;sk-..."
        :aria-label="m.key_pool_textarea_aria()"
      ...
      <p class="mt-1 text-xs text-ink-faint">
        {{ m.key_pool_count_hint({ count: poolLineCount }) }}
      </p>
    </div>

    <!-- 钥匙池策略 -->
    <div class="mt-4 flex flex-col gap-1.5">
      <span class="text-xs font-medium text-ink-soft">{{ m.key_pool_strategy_title() }}</span>
