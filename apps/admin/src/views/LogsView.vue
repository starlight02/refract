...
function exportLogs() {
  ...
  a.click()
  pageExported.value = true
  setTimeout(() => (pageExported.value = false), 2000)
  ...
  <div class="flex flex-col gap-5">
    <header class="flex flex-wrap items-end justify-between gap-4">
      ...
      <div class="flex flex-wrap items-center gap-2.5">
        ...
        >
          <AppIcon
            :name="pageExported ? 'check' : 'download'"
      ...
      <table v-else class="min-w-[900px] w-full text-sm">
        ...
        <tbody>
          <template v-for="log in store.items" :key="log.id">
            <tr
              class="cursor-pointer border-b border-ink/5 transition-colors duration-150 last:border-0 hover:bg-ink/[0.03] outline-none focus-visible:bg-accent/5 focus-visible:ring-1 focus-visible:ring-accent/30"
              role="button"
              tabindex="0"
              :aria-expanded="expanded.has(log.id)"
              :aria-controls="'log-detail-' + log.id"
              @click="toggleRow(log.id)"
              @keydown.enter="toggleRow(log.id)"
              ...
              >
                <span class="inline-flex items-center gap-1.5">
                  <AppIcon
                    ...
                    :class="expanded.has(log.id) ? 'rotate-90 text-accent' : 'text-ink-faint'"
                    aria-hidden="true"
                  />
                  <span>{{ shortTime(log.created_at) }}</span>
                </span>
              </td>
