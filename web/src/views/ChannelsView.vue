      ...
      <div>
        ...
        <p class="mt-1 text-sm text-ink-faint">
          ...
          <template v-if="suspendedCount > 0">
            · <span class="text-danger">熔断 {{ suspendedCount }} 个端点</span>
          </template>
        </p>
      </div>
          ...
          <div class="min-w-0 flex-1">
            ...
            >
              <div class="flex flex-wrap items-center gap-2">
                ...
                <span
                  >连续失败
                  <span class="tabular font-medium">{{ h.consecutive_fails }}</span> 次</span
                >
                <button
                  type="button"
                  class="ml-auto rounded-md bg-danger/15 px-2 py-1 font-medium hover:bg-danger/25 disabled:opacity-50"
    ...
    >
      ...
      >
        ...
        <button
          v-else
          type="button"
          class="rounded-full bg-danger px-3 py-1.5 text-xs font-medium text-white hover:brightness-105 disabled:opacity-50"
          :disabled="bulkBusy"
          @click="runBulk('delete')"
        >
