  ...
  <div class="flex min-h-screen text-ink">
    ...
    >
      <RouterLink :to="{ name: 'dashboard' }" class="flex items-center gap-2 no-underline">
        <span
          class="grid size-8 place-items-center rounded-[0.625rem] bg-linear-to-b from-accent-soft to-accent text-sm font-bold text-white shadow-[0_3px_10px_-2px_oklch(from_var(--color-accent)_l_c_h/0.4),inset_0_1px_0_0_oklch(100%_0_0/0.35)]"
          aria-hidden="true"
        >
          R
        </span>
      ...
      <button
        type="button"
        class="grid size-10 place-items-center rounded-lg text-ink-soft hover:bg-ink/8 hover:text-ink"
    ...
    <nav
      class="glass-thick fixed inset-x-3 bottom-3 z-30 grid h-16 grid-cols-5 px-1 md:hidden"
      aria-label="移动端主导航"
    >
      <RouterLink
